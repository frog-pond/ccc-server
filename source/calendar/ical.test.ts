...
test('ical function should filter events beyond maxEndDate', (t) => {
...
END:VCALENDAR`
	...
	t.is(events.length, 2, 'Should have filtered out the event beyond 2026')

	const eventSummaries = events.map((e) => e.summary)
	t.true(eventSummaries.includes('Event within range'))
	t.true(eventSummaries.includes('Event just at limit'))
	t.false(eventSummaries.includes('Event beyond maxEndDate'))
})
